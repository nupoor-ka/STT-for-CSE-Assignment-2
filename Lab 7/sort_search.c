        ...
        if (choice == 1) { // Bubble Sort
            ...
            for (width = 1; width < n; width *= 2) {
                for (i = 0; i < n; i += 2 * width) {
                    ...
                    l = left; r = mid + 1; j = left;
                    while (l <= mid && r <= right) {
                        if (arr[l] <= arr[r]) temp_arr[j++] = arr[l++];
                        else temp_arr[j++] = arr[r++];
                    }
                    while (l <= mid) temp_arr[j++] = arr[l++];
                    while (r <= right) temp_arr[j++] = arr[r++];
                    for (j = left; j <= right; j++) arr[j] = temp_arr[j];
                }
            }
            ...
            if (found) printf("Key %d found at index %d.\n", key, i);
            ...
            int temp_arr[100];
            for (i = 0; i < n; i++) temp_arr[i] = arr[i];
            for (i = 0; i < n - 1; i++) {
                for (j = 0; j < n - i - 1; j++) {
                    if (temp_arr[j] > temp_arr[j + 1]) {
                        temp = temp_arr[j]; temp_arr[j] = temp_arr[j + 1]; temp_arr[j + 1] = temp;
                    }
                }
            }
